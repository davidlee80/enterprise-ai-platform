-- Read one immutable Runtime Snapshot version for one tenant.
-- KEYS[1] runtime-snapshot:{tenant_id}:version:<config_version>
-- ARGV: tenant_id, config_version
...
if not string.match(config_version, "^[1-9][0-9]*$") then
  return response(false, "SNAPSHOT_VERSION_INVALID", { tenant_id = tenant_id, config_version = config_version })
end
if KEYS[1] ~= "runtime-snapshot:{" .. tenant_id .. "}:version:" .. config_version then
...
if key_type == "none" then
  return response(true, "SNAPSHOT_VERSION_MISSING", {
    tenant_id = tenant_id,
...
return response(true, "SNAPSHOT_VERSION_READ", {
  tenant_id = tenant_id,
  config_version = config_version,
  found = true,
