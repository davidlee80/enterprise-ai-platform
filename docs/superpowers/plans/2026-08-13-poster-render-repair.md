...
## 第二轮：对齐目标设计稿（2026-08-13）

用户提供了期望效果图后，按「全部 10 项 + 从参考图裁切素材 + CSS 简易地图底」的选择执行。

### 已达成

| # | 区块 | 结果 |
|---|---|---|
| 1 | 章节标题 | 深绿药丸 + 前置图标（定位钉/日历/灯泡/钱包），图标用 `filter: brightness(0) invert(1)` 染白 |
| 2 | 花销明细 | 横向四列 + 彩色图标（票/碗/公交/购物袋）+ 独立预算合计块（钱袋 + 34px 橙色金额） |
| 3 | 五天速览 | day 色浅底独立卡片行，主题名用 day 色，摘要可换两行，价格去边框 |
| 4 | 备注事项 | 行间浅虚线分隔 |
| 5 | 副标题 | 两侧横线装饰 |
| 6 | 底部空白 | 版式改为 hero 220px + 468px/301px 两行，65px 空白消除 |
| 7 | 路线图背景 | CSS 绘制浅绿陆地 + 不规则西湖水域 + 文字标注（替代缺失的地图素材） |
| 8 | 插画白底 | 边缘 flood fill 抠除，白墙保留，箭头与文字不再被遮挡 |
| 9 | Hero 文字 | 标题与副标题加同色光晕，塔身不再压掉「江南」 |
| 10 | 路线箭头 | 虚线贯通 1→2→3→4→5，marker 箭头可见 |

### 新增脚本

- `scripts/extract-icons.js` —— 从素材总图裁切。给粗框，脚本在框内自动收紧 bbox（不依赖手工像素对齐），近白转 alpha，输出 128px PNG；另可裁插画并用矩形遮罩抹掉蹭进来的邻近元素（石拱桥左上角的柳枝）。
- `scripts/make-transparent.js` —— 从图像四边 flood fill，只抠与边缘连通的背景白。这一点是关键：简单的「近白转透明」会把寺庙与民居的**白墙**一起掏空。

### 第二轮的新发现

**N4 — 插画文件名同样与内容错配。** `canal-bridge.webp` 实为**木栈道**，`wetland.webp` 实为**茶园绿团**，而 DAY3「运河人文线」需要的石拱桥根本不存在。已重命名为 `wetland-walkway.webp` / `tea-garden.webp`，并从素材总图裁出 `stone-bridge.webp`。至此文件名、`assetId`、tags、实际内容四者一致 —— 这类错配前后共踩三次（图标两次、插画一次）。

**N5 — 图标 semantic 存在跨用途冲突。** tips 与 budget 都有 `transport`，扁平表里会互相覆盖。改为带用途前缀（`tip-` / `budget-` / `section-`），模板按 `tip-${type}` 取用；`selectIcons` 不再按 `styleSet` 过滤，并新增「iconId 无重复」测试防止产物文件互相覆盖。

### 与目标稿的剩余差距（均为素材缺口）

1. **Hero 需要无缝全景图** —— 现有 `hangzhou-hero.webp` 是拱桥/游船/雷峰塔/民居的拼贴，元素之间有空隙，且塔身正落在副标题带上（现靠光晕缓解）。目标稿是一幅连续的远山—湖面—岸线水彩。
2. **路线图需要手绘地图素材** —— 现为 CSS 近似（浅绿陆地 + 椭圆水域）。目标稿有地块纹理、树木点缀与更自然的水岸形状。
3. **DAY4 缺夜间演出小图** —— 目标稿在 DAY4 节点旁有一张深蓝色演出照片。
4. **节点插画尺寸与排布** —— 目标稿插画更大（约 95px）且位置为手工排布；当前是统一规则（插画 66×56 居中 + 徽章 + DAY + 名称），放大会增加节点相互重叠的风险。
